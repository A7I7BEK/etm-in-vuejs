<template>
	<div class="sidebar-menu">
		<div class="sidebar-menu__inner">


			<router-link class="sidebar-menu__item"
						 v-if="can('TASK_READ')"
						 :to="{ name: 'mainTasks' }"
						 tag="div"
			>
				<a href="#">
					<div class="sidebar-menu__item__icon">
						<svg xmlns="http://www.w3.org/2000/svg" width="100%" viewBox="0 0 416 416" xml:space="preserve">
							<g>
								<polygon points="72.103,175.999 72.103,207.999 243.816,207.999 264.103,187.714 264.103,175.999 		"/>
							</g>
							<g>
								<polygon points="72.103,239.999 72.103,271.999 179.814,271.999 211.816,239.999 		"/>
							</g>
							<g>
								<rect x="72.104" y="303.998" width="96" height="32"/>
							</g>
							<g>
								<path d="M403.212,139.108l-22.628-22.627c-6.245-6.245-16.381-6.245-22.626,0c-6.03,6.03-11.568,11.568-16.877,16.877
			l45.255,45.255c5.35-5.35,10.902-10.902,16.876-16.877C409.459,155.49,409.459,145.354,403.212,139.108z"/>
							</g>
							<g>
								<path d="M329.797,144.7c-57.43,57.431-74.356,74.298-145.694,145.638c0,27.687,0,21.687,0,45.662
			c38.518-0.001,28.774,0.017,44.917-0.07c3.55-3.55,6.928-6.928,10.239-10.239l56.844-56.819V384h-256V96h32.002v32h191.998V96h32
			v59.717l32-31.999V96c0-17.674-14.326-32-32-32h-44.424c-5.926-6.583-13.538-11.62-22.284-14.136
			c-7.368-2.118-13.038-7.788-15.156-15.156C208.474,14.664,190.001,0,168.103,0c-21.898,0-40.37,14.664-46.136,34.707
			c-2.121,7.376-7.805,13.039-15.181,15.164C98.048,52.389,90.444,57.421,84.524,64H40.103c-17.673,0-32,14.326-32,32v288
			c0,17.673,14.327,32,32,32h256c17.674,0,32-14.327,32-32V236.885l46.95-46.93L329.797,144.7z M222.354,319.967
			c-5.962,0.034-6.66,0.034-21.938,0.032L200.103,320v-8.958v-14.078l5.214-5.215l22.627,22.628L222.354,319.967z M168.308,31.999
			c8.837,0,16,7.163,16,16c0,8.836-7.163,16-16,16s-16-7.164-16-16C152.308,39.162,159.472,31.999,168.308,31.999z"/>
							</g>
						</svg>
					</div>
					<span>{{ $tc('menu.task', 2) }}</span>
				</a>
			</router-link>


			<router-link class="sidebar-menu__item"
						 v-if="can('PROJECT_READ')"
						 :to="{ name: 'mainProjects' }"
						 tag="div"
			>
				<a href="#">
					<div class="sidebar-menu__item__icon">
						<svg width="100%" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 469.333 469.333" xml:space="preserve">
							<g>
								<path d="M458.667,0h-448C4.771,0,0,4.771,0,10.667v288c0,23.531,19.135,42.667,42.667,42.667h123.871l-37.986,113.958
				c-1.865,5.594,1.156,11.635,6.74,13.49c1.125,0.375,2.26,0.552,3.375,0.552c4.469,0,8.625-2.823,10.115-7.292l40.236-120.708H224
				v117.333c0,5.896,4.771,10.667,10.667,10.667s10.667-4.771,10.667-10.667V341.333h34.983l40.236,120.708
				c1.49,4.469,5.646,7.292,10.115,7.292c1.115,0,2.25-0.177,3.375-0.552c5.583-1.854,8.604-7.896,6.74-13.49l-37.986-113.958
				h123.871c23.531,0,42.667-19.135,42.667-42.667v-288C469.333,4.771,464.563,0,458.667,0z M426.667,298.667h-384v-256h384V298.667
				z"/>
								<path d="M96,277.333h64c5.896,0,10.667-4.771,10.667-10.667v-85.333c0-5.896-4.771-10.667-10.667-10.667H96
				c-5.896,0-10.667,4.771-10.667,10.667v85.333C85.333,272.562,90.104,277.333,96,277.333z"/>
								<path d="M202.667,277.333h64c5.896,0,10.667-4.771,10.667-10.667V117.333c0-5.896-4.771-10.667-10.667-10.667h-64
				c-5.896,0-10.667,4.771-10.667,10.667v149.333C192,272.562,196.771,277.333,202.667,277.333z"/>
								<path d="M309.333,277.333h64c5.896,0,10.667-4.771,10.667-10.667v-192C384,68.771,379.229,64,373.333,64h-64
				c-5.896,0-10.667,4.771-10.667,10.667v192C298.667,272.562,303.437,277.333,309.333,277.333z"/>
							</g>
						</svg>
					</div>
					<span>{{ $tc('menu.project', 2) }}</span>
				</a>
			</router-link>


			<router-link class="sidebar-menu__item"
						 v-if="can('GROUP_READ')"
						 :to="{ name: 'mainTeams' }"
						 tag="div"
			>
				<a href="#">
					<div class="sidebar-menu__item__icon">
						<svg xmlns="http://www.w3.org/2000/svg" width="19"
							 viewBox="0 0 511.999 511.999" style="enable-background:new 0 0 511.999 511.999;" xml:space="preserve">
							<path d="M438.09,273.32h-39.596c4.036,11.05,6.241,22.975,6.241,35.404v149.65c0,5.182-0.902,10.156-2.543,14.782h65.461
			c24.453,0,44.346-19.894,44.346-44.346v-81.581C512,306.476,478.844,273.32,438.09,273.32z"/>
							<path d="M107.265,308.725c0-12.43,2.205-24.354,6.241-35.404H73.91c-40.754,0-73.91,33.156-73.91,73.91v81.581
			c0,24.452,19.893,44.346,44.346,44.346h65.462c-1.641-4.628-2.543-9.601-2.543-14.783V308.725z"/>
							<path d="M301.261,234.815h-90.522c-40.754,0-73.91,33.156-73.91,73.91v149.65c0,8.163,6.618,14.782,14.782,14.782h208.778
			c8.164,0,14.782-6.618,14.782-14.782v-149.65C375.171,267.971,342.015,234.815,301.261,234.815z"/>
							<path d="M256,38.84c-49.012,0-88.886,39.874-88.886,88.887c0,33.245,18.349,62.28,45.447,77.524
			c12.853,7.23,27.671,11.362,43.439,11.362c15.768,0,30.586-4.132,43.439-11.362c27.099-15.244,45.447-44.28,45.447-77.524
			C344.886,78.715,305.012,38.84,256,38.84z"/>
							<path d="M99.918,121.689c-36.655,0-66.475,29.82-66.475,66.475c0,36.655,29.82,66.475,66.475,66.475
			c9.298,0,18.152-1.926,26.195-5.388c13.906-5.987,25.372-16.585,32.467-29.86c4.98-9.317,7.813-19.946,7.813-31.227
			C166.393,151.51,136.573,121.689,99.918,121.689z"/>
							<path d="M412.082,121.689c-36.655,0-66.475,29.82-66.475,66.475c0,11.282,2.833,21.911,7.813,31.227
			c7.095,13.276,18.561,23.874,32.467,29.86c8.043,3.462,16.897,5.388,26.195,5.388c36.655,0,66.475-29.82,66.475-66.475
			C478.557,151.509,448.737,121.689,412.082,121.689z"/>
						</svg>
					</div>
					<span>{{ $tc('menu.team', 2) }}</span>
				</a>
			</router-link>


			<router-link class="sidebar-menu__item"
						 v-if="can('EMPLOYEE_READ')"
						 :to="{ name: 'mainUsers' }"
						 tag="div"
			>
				<a href="#">
					<div class="sidebar-menu__item__icon">
						<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16">
							<path d="M12,12A4,4,0,1,0,8,8,4,4,0,0,0,12,12Zm0,2c-2.67,0-8,1.34-8,4v2H20V18C20,15.34,14.67,14,12,14Z" transform="translate(-4 -4)"></path>
						</svg>
					</div>
					<span>{{ $tc('menu.user', 2) }}</span>
				</a>
			</router-link>


			<router-link class="sidebar-menu__item"
						 v-if="can('ROLE_READ')"
						 :to="{ name: 'mainRoles' }"
						 tag="div"
			>
				<a href="#">
					<div class="sidebar-menu__item__icon">
						<svg xmlns="http://www.w3.org/2000/svg" width="100"
							 viewBox="0 0 474.565 474.565" style="enable-background:new 0 0 474.565 474.565;" xml:space="preserve">
							<path d="M255.204,102.3c-0.606-11.321-12.176-9.395-23.465-9.395C240.078,95.126,247.967,98.216,255.204,102.3z"/>
							<path d="M134.524,73.928c-43.825,0-63.997,55.471-28.963,83.37c11.943-31.89,35.718-54.788,66.886-63.826
		C163.921,81.685,150.146,73.928,134.524,73.928z"/>
							<path d="M43.987,148.617c1.786,5.731,4.1,11.229,6.849,16.438L36.44,179.459c-3.866,3.866-3.866,10.141,0,14.015l25.375,25.383
		c1.848,1.848,4.38,2.888,7.019,2.888c2.61,0,5.125-1.04,7.005-2.888l14.38-14.404c2.158,1.142,4.55,1.842,6.785,2.827
		c0-0.164-0.016-0.334-0.016-0.498c0-11.771,1.352-22.875,3.759-33.302c-17.362-11.174-28.947-30.57-28.947-52.715
		c0-34.592,28.139-62.739,62.723-62.739c23.418,0,43.637,13.037,54.43,32.084c11.523-1.429,22.347-1.429,35.376,1.033
		c-1.676-5.07-3.648-10.032-6.118-14.683l14.396-14.411c1.878-1.856,2.918-4.38,2.918-7.004c0-2.625-1.04-5.148-2.918-7.004
		l-25.361-25.367c-1.94-1.941-4.472-2.904-7.003-2.904c-2.532,0-5.063,0.963-6.989,2.904l-14.442,14.411
		c-5.217-2.764-10.699-5.078-16.444-6.825V9.9c0-5.466-4.411-9.9-9.893-9.9h-35.888c-5.451,0-9.909,4.434-9.909,9.9v20.359
		c-5.73,1.747-11.213,4.061-16.446,6.825L75.839,22.689c-1.942-1.941-4.473-2.904-7.005-2.904c-2.531,0-5.077,0.963-7.003,2.896
		L36.44,48.048c-1.848,1.864-2.888,4.379-2.888,7.012c0,2.632,1.04,5.148,2.888,7.004l14.396,14.403
		c-2.75,5.218-5.063,10.708-6.817,16.438H23.675c-5.482,0-9.909,4.441-9.909,9.915v35.889c0,5.458,4.427,9.908,9.909,9.908H43.987z"
							/>
							<path d="M354.871,340.654c15.872-8.705,26.773-25.367,26.773-44.703c0-28.217-22.967-51.168-51.184-51.168
		c-9.923,0-19.118,2.966-26.975,7.873c-4.705,18.728-12.113,36.642-21.803,52.202C309.152,310.022,334.357,322.531,354.871,340.654z
		"/>
							<path d="M460.782,276.588c0-5.909-4.799-10.693-10.685-10.693H428.14c-1.896-6.189-4.411-12.121-7.393-17.75l15.544-15.544
		c2.02-2.004,3.137-4.721,3.137-7.555c0-2.835-1.118-5.553-3.137-7.563l-27.363-27.371c-2.08-2.09-4.829-3.138-7.561-3.138
		c-2.734,0-5.467,1.048-7.547,3.138l-15.576,15.552c-5.623-2.982-11.539-5.481-17.751-7.369v-21.958
		c0-5.901-4.768-10.685-10.669-10.685H311.11c-2.594,0-4.877,1.04-6.739,2.578c3.26,11.895,5.046,24.793,5.046,38.552
		c0,8.735-0.682,17.604-1.956,26.423c7.205-2.656,14.876-4.324,22.999-4.324c36.99,0,67.086,30.089,67.086,67.07
		c0,23.637-12.345,44.353-30.872,56.303c13.48,14.784,24.195,32.324,31.168,51.976c1.148,0.396,2.344,0.684,3.54,0.684
		c2.733,0,5.467-1.04,7.563-3.13l27.379-27.371c2.004-2.004,3.106-4.721,3.106-7.555s-1.102-5.551-3.106-7.563l-15.576-15.552
		c2.982-5.621,5.497-11.555,7.393-17.75h21.957c2.826,0,5.575-1.118,7.563-3.138c2.004-1.996,3.138-4.72,3.138-7.555
		L460.782,276.588z"/>
							<path d="M376.038,413.906c-16.602-48.848-60.471-82.445-111.113-87.018c-16.958,17.958-37.954,29.351-61.731,29.351
		c-23.759,0-44.771-11.392-61.713-29.351c-50.672,4.573-94.543,38.17-111.145,87.026l-9.177,27.013
		c-2.625,7.773-1.368,16.338,3.416,23.007c4.783,6.671,12.486,10.631,20.685,10.631h315.853c8.215,0,15.918-3.96,20.702-10.631
		c4.767-6.669,6.041-15.234,3.4-23.007L376.038,413.906z"/>
							<path d="M120.842,206.782c0,60.589,36.883,125.603,82.352,125.603c45.487,0,82.368-65.014,82.368-125.603
		C285.563,81.188,120.842,80.939,120.842,206.782z"/>
						</svg>
					</div>
					<span>{{ $tc('menu.role', 2) }}</span>
				</a>
			</router-link>


			<div class="sidebar-menu__item"
				 data-custom-accordion
				 v-if="can('REPORT_KANBAN_PROJECT_MANAGER_AND_MEMBER')
				 || can('REPORT_TRELLO_PROJECT_MANAGER_AND_MEMBER')
				 || can('REPORT_CHART_KANBAN_PROJECT')
				 || can('REPORT_CHART_TRELLO_PROJECT')
				 || can('REPORT_KANBAN_TRELLO_PROJECT_WITH_USER')
				 || can('REPORT_TIME_LEFT')"
				 :class="{'active': computedPageName.includes('mainReports')}"
			>
				<a href="#" data-custom-accordion-btn>
					<div class="sidebar-menu__item__icon">
						<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16">
							<path d="M15.429,4H12.571V.571A.571.571,0,0,0,12,0H.571A.571.571,0,0,0,0,.571V13.714A2.286,2.286,0,0,0,2.286,16H13.714A2.286,2.286,0,0,0,16,13.714V4.571A.571.571,0,0,0,15.429,4ZM4.571,2.857H8A.571.571,0,1,1,8,4H4.571a.571.571,0,1,1,0-1.143ZM9.714,13.714H2.857a.571.571,0,1,1,0-1.143H9.714a.571.571,0,1,1,0,1.143Zm0-2.286H2.857a.571.571,0,1,1,0-1.143H9.714a.571.571,0,1,1,0,1.143Zm0-2.286H2.857A.571.571,0,0,1,2.857,8H9.714a.571.571,0,0,1,0,1.143Zm0-2.286H2.857a.571.571,0,1,1,0-1.143H9.714a.571.571,0,1,1,0,1.143Zm5.143,6.857a1.143,1.143,0,1,1-2.286,0V5.143h2.286v8.571Z"/>
						</svg>
					</div>
					<span>{{ $tc('menu.report', 2) }}</span>
				</a>


				<div data-custom-accordion-body
					 :style="computedPageName.includes('mainReports') ? 'display: block' : 'display: none'"
				>
					<div class="sidebar-settings">
						<div class="sidebar-settings__inner">

							<div class="sidebar-settings__item"
								 v-if="can('REPORT_CHART_KANBAN_PROJECT') && can('REPORT_CHART_TRELLO_PROJECT')"
							>
								<router-link :to="{ name: 'mainReportsProjectOverview' }">
									<div class="settings-icon">
										<svg xmlns="http://www.w3.org/2000/svg" width="16" height="17.778" viewBox="0 0 16 17.778">
											<path d="M17.222,2.778H13.507a2.656,2.656,0,0,0-5.013,0H4.778A1.783,1.783,0,0,0,3,4.556V17a1.783,1.783,0,0,0,1.778,1.778H17.222A1.783,1.783,0,0,0,19,17V4.556A1.783,1.783,0,0,0,17.222,2.778ZM11,2.778a.889.889,0,1,1-.889.889A.892.892,0,0,1,11,2.778ZM9.222,15.222,5.667,11.667,6.92,10.413l2.3,2.293L15.08,6.849l1.253,1.262Z"
												  transform="translate(-3 -1)"/>
										</svg>
									</div>
									<h5>{{ $t('menu.projectOverview') }}</h5>
								</router-link>
							</div>


							<div class="sidebar-settings__item"
								 v-if="can('REPORT_TIME_LEFT')"
							>
								<router-link :to="{ name: 'mainReportsTimeSeries' }">
									<div class="settings-icon">
										<svg xmlns="http://www.w3.org/2000/svg" width="16" height="17.778" viewBox="0 0 16 17.778">
											<path d="M17.222,2.778H13.507a2.656,2.656,0,0,0-5.013,0H4.778A1.783,1.783,0,0,0,3,4.556V17a1.783,1.783,0,0,0,1.778,1.778H17.222A1.783,1.783,0,0,0,19,17V4.556A1.783,1.783,0,0,0,17.222,2.778ZM11,2.778a.889.889,0,1,1-.889.889A.892.892,0,0,1,11,2.778ZM9.222,15.222,5.667,11.667,6.92,10.413l2.3,2.293L15.08,6.849l1.253,1.262Z"
												  transform="translate(-3 -1)"/>
										</svg>
									</div>
									<h5>{{ $t('menu.timeSeries') }}</h5>
								</router-link>
							</div>


							<div class="sidebar-settings__item">
								<router-link :to="{ name: 'mainReportsTimeFrame' }">
									<div class="settings-icon">
										<svg xmlns="http://www.w3.org/2000/svg" width="16" height="17.778" viewBox="0 0 16 17.778">
											<path d="M17.222,2.778H13.507a2.656,2.656,0,0,0-5.013,0H4.778A1.783,1.783,0,0,0,3,4.556V17a1.783,1.783,0,0,0,1.778,1.778H17.222A1.783,1.783,0,0,0,19,17V4.556A1.783,1.783,0,0,0,17.222,2.778ZM11,2.778a.889.889,0,1,1-.889.889A.892.892,0,0,1,11,2.778ZM9.222,15.222,5.667,11.667,6.92,10.413l2.3,2.293L15.08,6.849l1.253,1.262Z"
												  transform="translate(-3 -1)"/>
										</svg>
									</div>
									<h5>{{ $t('menu.timeFrame') }}</h5>
								</router-link>
							</div>


							<div class="sidebar-settings__item"
								 v-if="can('REPORT_KANBAN_TRELLO_PROJECT_WITH_USER')"
							>
								<router-link :to="{ name: 'mainReportsProjectDetailsTask' }">
									<div class="settings-icon">
										<svg xmlns="http://www.w3.org/2000/svg" width="16" height="17.778" viewBox="0 0 16 17.778">
											<path d="M17.222,2.778H13.507a2.656,2.656,0,0,0-5.013,0H4.778A1.783,1.783,0,0,0,3,4.556V17a1.783,1.783,0,0,0,1.778,1.778H17.222A1.783,1.783,0,0,0,19,17V4.556A1.783,1.783,0,0,0,17.222,2.778ZM11,2.778a.889.889,0,1,1-.889.889A.892.892,0,0,1,11,2.778ZM9.222,15.222,5.667,11.667,6.92,10.413l2.3,2.293L15.08,6.849l1.253,1.262Z"
												  transform="translate(-3 -1)"/>
										</svg>
									</div>
									<h5>{{ $t('menu.projectDetailsTask') }}</h5>
								</router-link>
							</div>


							<div class="sidebar-settings__item"
								 v-if="can('REPORT_KANBAN_PROJECT_MANAGER_AND_MEMBER') && can('REPORT_TRELLO_PROJECT_MANAGER_AND_MEMBER')"
							>
								<router-link :to="{ name: 'mainReportsProjectDetailsManager' }">
									<div class="settings-icon">
										<svg xmlns="http://www.w3.org/2000/svg" width="16" height="17.778" viewBox="0 0 16 17.778">
											<path d="M17.222,2.778H13.507a2.656,2.656,0,0,0-5.013,0H4.778A1.783,1.783,0,0,0,3,4.556V17a1.783,1.783,0,0,0,1.778,1.778H17.222A1.783,1.783,0,0,0,19,17V4.556A1.783,1.783,0,0,0,17.222,2.778ZM11,2.778a.889.889,0,1,1-.889.889A.892.892,0,0,1,11,2.778ZM9.222,15.222,5.667,11.667,6.92,10.413l2.3,2.293L15.08,6.849l1.253,1.262Z"
												  transform="translate(-3 -1)"/>
										</svg>
									</div>
									<h5>{{ $t('menu.projectDetailsManager') }}</h5>
								</router-link>
							</div>


							<div class="sidebar-settings__item"
								 v-if="can('REPORT_KANBAN_PROJECT_MANAGER_AND_MEMBER') && can('REPORT_TRELLO_PROJECT_MANAGER_AND_MEMBER')"
							>
								<router-link :to="{ name: 'mainReportsProjectDetailsEmployee' }">
									<div class="settings-icon">
										<svg xmlns="http://www.w3.org/2000/svg" width="16" height="17.778" viewBox="0 0 16 17.778">
											<path d="M17.222,2.778H13.507a2.656,2.656,0,0,0-5.013,0H4.778A1.783,1.783,0,0,0,3,4.556V17a1.783,1.783,0,0,0,1.778,1.778H17.222A1.783,1.783,0,0,0,19,17V4.556A1.783,1.783,0,0,0,17.222,2.778ZM11,2.778a.889.889,0,1,1-.889.889A.892.892,0,0,1,11,2.778ZM9.222,15.222,5.667,11.667,6.92,10.413l2.3,2.293L15.08,6.849l1.253,1.262Z"
												  transform="translate(-3 -1)"/>
										</svg>
									</div>
									<h5>{{ $t('menu.projectDetailsEmployee') }}</h5>
								</router-link>
							</div>

						</div>
					</div>
				</div>
			</div>


			<router-link class="sidebar-menu__item"
						 v-if="$store.state.userProfile.systemAdmin && can('PERMISSION_READ')"
						 :to="{ name: 'mainPermissions' }"
						 tag="div"
			>
				<a href="#">
					<div class="sidebar-menu__item__icon">
						<svg xmlns="http://www.w3.org/2000/svg" width="12.19" height="16" viewBox="0 0 12.19 16">
							<path d="M14.667,6.333H13.9V4.81a3.81,3.81,0,1,0-7.619,0V6.333H5.524A1.528,1.528,0,0,0,4,7.857v7.619A1.528,1.528,0,0,0,5.524,17h9.143a1.528,1.528,0,0,0,1.524-1.524V7.857A1.528,1.528,0,0,0,14.667,6.333ZM10.1,13.19a1.524,1.524,0,1,1,1.524-1.524A1.528,1.528,0,0,1,10.1,13.19Zm2.362-6.857H7.733V4.81a2.362,2.362,0,0,1,4.724,0Z"
								  transform="translate(-4 -1)"></path>
						</svg>
					</div>
					<span>{{ $tc('menu.permission', 2) }}</span>
				</a>
			</router-link>


			<router-link class="sidebar-menu__item"
						 v-if="$store.state.userProfile.systemAdmin && can('ORGANIZATION_READ')"
						 :to="{ name: 'mainOrganizations' }"
						 tag="div"
			>
				<a href="#">
					<div class="sidebar-menu__item__icon">
						<svg xmlns="http://www.w3.org/2000/svg" width="16" height="15" viewBox="0 0 16 15">
							<path d="M16.4,5.158H13.2V3.579A1.584,1.584,0,0,0,11.6,2H8.4A1.584,1.584,0,0,0,6.8,3.579V5.158H3.6A1.577,1.577,0,0,0,2.008,6.737L2,15.421A1.584,1.584,0,0,0,3.6,17H16.4A1.584,1.584,0,0,0,18,15.421V6.737A1.584,1.584,0,0,0,16.4,5.158Zm-4.8,0H8.4V3.579h3.2Z"
								  transform="translate(-2 -2)"/>
						</svg>
					</div>
					<span>{{ $tc('menu.organization', 2) }}</span>
				</a>
			</router-link>

		</div>
	</div>
</template>

<script>
	export default {
		name: 'SidebarMenu',
		computed: {
			computedPageName()
			{
				return this.$route.name;
			}
		},
	};
</script>