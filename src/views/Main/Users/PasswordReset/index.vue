<template>
	<div class="az_base_box" v-if="can('EMPLOYEE_PASSWORD_CHANGE')">

		<div class="az_crud_ttl mb-5">
			<div class="az_crud_ttl_left">
				<router-link class="btn az_base_btn btn-default icon mr-3" :to="{ name: 'mainUsers' }">
					<i class="fa fa-arrow-left"></i>
				</router-link>

				<i18n class="az_crud_ttl_txt" tag="h3" path="crudPasswordReset">
					<template #name>{{ user.userName }}</template>
				</i18n>
			</div>
		</div>
	...
	export default {
		...
		methods: {
			...
			submit()
			{
				...
				this.$api
					.put('employees/password/change/' + this.id, {
