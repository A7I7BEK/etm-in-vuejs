<template>
	<div>
		<div class="az_ovw_select">
			<select class="form-control az_base_inp" v-model="projectSelected">
		...
		<template v-if="projectDataList.length > 0">
					...
					<tr v-for="item in projectDataList">
						<td>
							<div class="d-flex align-items-center">
								<div class="az_ovw_color mr-3" :style="{ 'background': item.color }"></div> {{ item.columnName }}
							</div>
						</td>
						<td>{{ item.count }}</td>
					</tr>
			...
			<div class="az_overview_chart_kan" v-if="series.length > 0">
				<vue-apex-charts type="pie" width="100%" :options="chartOptions" :series="series" :key="chartKey"/>
			</div>
		</template>
