<template>
	<div v-if="options && options.end">
		<div class="d-flex mt-4">
			<input type="text" class="form-control az_base_inp" style="max-width: 300px;" :placeholder="$t('search')" v-model="searchText">
		</div>
		...
		<template v-if="dataList.length > 0">
			<base-report-table class="mt-2">
				<template #head>
					<tr>
						...
						<th v-for="(item, index) in dataList[0].reports" :key="index">{{ $t(item.name) }}</th>
			...
			<div class="az_report_foot" v-if="can('REPORT_TIME_LEFT_UPLOAD')">
			...
			GetDataAll(type, page) {
				...
				this.$api
					.get('reports/hours/by/timeleft', {
						params: {
							'typeDateInterval': 'WEEKLY_INTERVAL',
