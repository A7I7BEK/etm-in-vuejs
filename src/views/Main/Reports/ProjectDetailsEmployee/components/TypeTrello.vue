	...
	<div>
		<div class="az_report_nav">
			<div class="az_report_nav_it">
				...
				<select class="form-control az_base_inp" v-model="selectedData.project">
			...
			<div class="az_report_nav_it">
				...
				<select class="form-control az_base_inp" v-model="selectedData.member">
					<option :value="null">{{$t('choose')}}</option>
					<option v-for="item in memberList" :value="item.id">{{ item.firstName }} {{ item.lastName }}</option>
				</select>
			</div>
		</div>
	...
	export default {
		...
		methods: {
			...
			GetDataAll(type, page) {
				...
				this.$api
					.get('reports/by/project/trello/managersAndMember', {
						params: {
							'projectId': this.selectedData.project,
