	...
	>
		<template #table v-if="Object.keys(model).length > 0">
			...
			<table class="table az_crud_read_tb">
					...
					<td>
						<ul class="az_crud_read_tb_ls">
							<li v-for="item in model.members">
								...
								<i class="fa fa-user-secret ml-2" v-if="item.employee.isTeamLeader" :title="$t('teamLeader')"></i>
								<i class="fa fa-star ml-2" v-if="item.employee.id === model.manager.id" :title="$t('manager')"></i>
							</li>
						</ul>
					</td>
				</tr>
				...
				<tr>
					<th>{{ $t('manager') }}</th>
					<td>{{ model.manager.firstName + ' ' + model.manager.lastName + ' ' + model.manager.middleName }}</td>
				</tr>
				</tbody>
			</table>
