	...
	<div>
		<form @submit.prevent="SendOTP">
			<div class="d-flex align-items-center justify-content-center mb-4 us-n">
				<label class="auth_form_switch_opt"
				...
				<div class="custom-control custom-switch auth_form_switch">
					...
					<label class="custom-control-label" for="contactTypeSwitch"></label>
				</div>
			...
			<div class="auth_form_group" v-show="otpType === 'email'">
			...
			<button class="btn az_base_btn btn-primary btn-block" type="submit">{{ $t('submit') }}</button>
		</form>
			...
			SendOTP()
			{
				...
				this.$api
					.post('auth/forgot/password/send/otp', {
						paramForSendingOtp: this.model[this.otpType],
					})
					.then(response => {
						this.$emit('emit:data', {
