<template>
	<div class="mw-menu__item mw-menu__checklist custom-dropdown__main" data-custom-drop>
		...
		<div class="custom-dropdown right w300" data-custom-drop-body>
			<div class="member-invite__top"><h3>{{$t('share')}}</h3></div>
			...
			<div class="member-invite__content">
				...
				<div class="form-group us-n">
					<div class="custom-control custom-checkbox az_task_share_custom_chk">
						...
						>
						<label class="custom-control-label" style="padding-top: 2px;" for="checkboxEmail">{{$t('shareViaEmail')}}</label>
					</div>
					...
					<div class="custom-control custom-checkbox az_task_share_custom_chk">
						...
						>
						<label class="custom-control-label" style="padding-top: 2px;" for="checkboxTelegram">{{$t('shareViaTelegram')}}</label>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<input class="form-control" type="text" v-model="employeeSearch" :placeholder="$t('search')">
				</div>

				<div class="form-group" v-if="employeeList">
					<ul class="az_mem_inv_ls us-n" v-if="employeeList.length > 0">

						<li v-for="item in employeeList" :class="{'active': item.checked}" @click="ToggleEmployee(item)">
							<div class="az_mem_inv">
								...
								<div class="az_mem_inv_txt">
									...
									<div class="az_mem_inv_uname">{{ item.userName }}</div>
								</div>
							</div>
						</li>
				...
				<div class="justify-content-end mt-3" style="display: flex;" v-show="employeeSelectedList.length > 0">
					<button type="button" class="member-invite-btn button-effect" @click="Share">{{$t('share')}}</button>
				</div>
