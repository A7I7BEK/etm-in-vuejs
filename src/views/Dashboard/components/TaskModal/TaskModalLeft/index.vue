		...
		<div class="modal-header">
			<div class="mw-title az_task_mdl_hd">
				<div class="d-flex align-items-center az_task_mdl_hd_text">
					<div class="mr-2" v-if="can('TASK_UPDATE')">
						<div class="d-flex" v-if="taskNameEditMode">
							<button type="button" class="btn btn-sm az_r_menu_ttl_btn" @click="EditTaskName">
								<b class="fa fa-check"></b>
							</button>
							<button type="button" class="btn btn-sm az_r_menu_ttl_btn" @click="taskNameEditMode = false;">
					...
					<div class="flex-fill">
						<form @submit.prevent="EditTaskName" v-if="taskNameEditMode">
							<input type="text"
						...
						<h5 class="modal-title new-team" v-else>{{ $store.state.taskModalData.name }}</h5>
					</div>
				</div>

				<button type="button" class="mw-close" data-dismiss="modal" aria-label="Close">
					<p>{{ $t('close') }}</p>
			...
			<div class="mw-title__bottom">
				...
				<div class="mw-title__btm__right">
					...
					<span v-if="$store.state.taskModalData.taskLevelType">{{ $t($store.state.taskModalData.taskLevelType.value) }}</span>
					<span v-else>{{$t('notSet')}}</span>
					<!--<div-->
					<!--v-if="$store.state.taskModalData.taskLevelType.value"-->
					<!--class="level-status"-->
					<!--:class="[-->
					<!--{'danger': $store.state.taskModalData.taskLevelType.value === $store.state.TASK_LEVEL_TYPE.HIGH},-->
					<!--{'warning': $store.state.taskModalData.taskLevelType.value === $store.state.TASK_LEVEL_TYPE.NORMAL},-->
					<!--{'success': $store.state.taskModalData.taskLevelType.value === $store.state.TASK_LEVEL_TYPE.LOW}-->
					<!--]"-->
					<!--&gt;-->
