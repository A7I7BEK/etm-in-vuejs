	...
	<div class="mw-checklist">
		<div class="mw-checklist__top">
			...
			<div class="mw-checklist__edit us-n">
				<div class="mw-checklist__edit__inner mw-checklist__hide" @click="checkedItemToggle = !checkedItemToggle">
					...
					<div class="mw-checklist__edit__icon">
						<i class="fa fa-eye-slash" v-if="checkedItemToggle"></i>
						<i class="fa fa-eye" v-else></i>
					</div>
				</div>

				<div class="ml-3 mw-checklist__edit__inner mw-checklist__delete" @click="DeleteGroup(checkListData.id)"
		...
		<div class="mw-checklist__content">
			<div class="mw-checklist__progressbar">
				...
				<div class="mw-checklist__progressbar__main">
					<div class="erj-progress" >
						<div class="progress-value" :style="{'width': checkListData.percent + '%'}"></div>
					</div>
				</div>
			</div>
			...
			<div v-if="can('TASK_CHECK_READ')">
				<template v-if="checkedItemToggle">
					<div class="mw-checklist__item" v-for="item in checkListData.checkList" :key="item.id">
						<div class="mw-checklist__item__inner">
							<label class="form-check-label us-n cur-p">
								<input type="checkbox" hidden v-model="item.checked" @change="Update(item)" :disabled="!can('TASK_UPDATE') || !can('TASK_CHECK_UPDATE')">
						...
						<div class="mw-checkbox__item__edit dropdown" v-if="can('TASK_UPDATE') && can('TASK_CHECK_DELETE')">
							...
							<div class="dropdown-menu">
								<a class="dropdown-item" href="#" @click.prevent="Delete(item.id)">{{$t('delete')}}</a>
							</div>
						</div>
					</div>
				</template>
				...
				<template v-else>
					<div class="mw-checklist__item" v-for="item in computedUncheckData.uncheckedList" :key="item.id">
						<div class="mw-checklist__item__inner">
							<label class="form-check-label us-n cur-p">
								<input type="checkbox" hidden v-model="item.checked" @change="Update(item)" :disabled="!can('TASK_UPDATE') || !can('TASK_CHECK_UPDATE')">
								<div class="checklist-icon"></div>
			...
			<form @submit.prevent="Create" v-if="can('TASK_UPDATE') && can('TASK_CHECK_CREATE')">
				<div class="form-group mw-checklist__bottom__input">
					...
					<div class="mw-checklist__bottom">
						<div class="member-invite custom-dropdown__main us-n m-0 mt-3" data-custom-drop>
						...
						<div class="checklist__bottom__right">
							<button type="button" class="checklist__bottom__close button-effect" @click="ClearData">{{$t('cancel')}}</button>
							<button type="submit" class="checklist__bottom__save button-effect">{{$t('save')}}</button>
						</div>
					</div>
				</div>
			</form>
	...
	export default {
		...
		methods: {
			GetChecklistGroupOne() {
				this.$api
					.get('checkListGroups/' + this.checkListData.id)
					.then(response => {
