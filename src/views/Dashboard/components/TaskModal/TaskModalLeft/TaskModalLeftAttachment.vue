	...
	<div class="mw-attachment">
		<div class="mw-attachment__top">
			...
			<div class="az_attach_screen us-n">
				<input class="inp" id="attachmentScreenshot" type="text" @paste="SaveClipboardImage">
				<label class="lbl" for="attachmentScreenshot">{{ $t('pasteScreenshot') }}</label>
			</div>
		</div>


		<div class="mw-attachment__content" v-if="$store.state.taskModalData.attachments.length > 0">
			<div class="mw-attachment__item" v-for="item in $store.state.taskModalData.attachments" :key="item.id">
				<div class="mw-attachment__item__photo">
					...
					<a v-else :href="$store.state.url + item.url" download>
				...
				<div class="mw-attachment__main">
					...
					<div class="attachment-main__bottom" v-if="can('TASK_UPDATE')">
						<div class="attachment-change__block" data-custom-drop>
							<div class="attachment-change__block__inner" data-custom-drop-btn @click="attachmentName = item.name">
								<span>{{$t('edit')}}</span>
								<div>
							...
							<div class="attachment-change__inner" data-custom-drop-body>
								...
								<div class="attachment-change__body">
									<form @submit.prevent="Update(item.id)">
										<div class="form-group">
											...
											<button class="button-effect" type="submit">{{$t('save')}}</button>
										</div>
									</form>
								</div>
							</div>
						...
						<div class="attachment-delete__block" @click="Delete(item.id)">
							<span>{{$t('delete')}}</span>
