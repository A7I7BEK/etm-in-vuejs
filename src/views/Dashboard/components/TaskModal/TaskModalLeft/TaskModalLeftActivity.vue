<template>
	<div class="mw-actions">
		<div class="mw-actions__home">
			...
			<div class="mw-accordion" data-custom-accordion>
				<div class="link px-0 d-flex align-items-center justify-content-between" data-custom-accordion-btn>
					<div>
				...
				<div class="mw-submenu__main_bx" style="display: none;" data-custom-accordion-body>
					<div class="mw-submenu__main">

						<div class="action__content__top" v-if="can('TASK_UPDATE') && can('TASK_COMMENT_CREATE')">
							<div class="mw-action__ava" v-if="$store.state.userProfile.photoUrl">
								<img :src="$store.state.url + $store.state.userProfile.photoUrl" alt="">
							</div>
									...
									<div class="action-save">
										<div class="d-flex align-items-center">
											<div class="form-group mb-0">
												<select class="form-control" v-model="commentTypeSelectedId">
													<option v-for="item in $store.state.projectData.taskCommentTypes" :key="item.id" :value="item.id">
														<template v-if="item.value === $store.state.TASK_COMMENT_TYPE.PROBLEM">{{$t('problem')}}</template>
														<template v-else-if="item.value === $store.state.TASK_COMMENT_TYPE.SOLUTION">{{$t('solution')}}</template>
														<template v-else>{{$t('information')}}</template>
													</option>
												</select>
											</div>

											<div class="member-invite custom-dropdown__main us-n" data-custom-drop>
												<div class="mw-checklist__findUser button-effect mt-0 d-flex align-items-center" data-custom-drop-btn>
													@{{$t('workers')}}<strong class="num" v-if="employeeListChecked.length > 0">{{ employeeListChecked.length }}</strong>
												</div>
												...
												<div class="custom-dropdown" data-custom-drop-body>
													...
													<div class="member-invite__content">
														...
														<div class="d-flex flex-wrap">
															...
															>
																<input type="checkbox" :value="item.employee.id" hidden v-model="employeeListChecked">
																...
																<div class="boardofusers-ava">
																	...
																	<img v-else :src="$store.state.url + item.employee.photoUrl">
																</div>
										...
										<div class="d-flex">
											<button type="button" class="modal-btn__save cancel button-effect mr-3" @click="ClearSaveData">{{$t('cancel')}}</button>
											<button type="submit" class="modal-btn__save button-effect">{{$t('save')}}</button>
										</div>
									</div>
								</form>
							</div>
						...
						<ul class="mw-submenu mw-comment__main" v-if="can('TASK_COMMENT_READ')">
							<li class="mw-action__item mw-comment__item" v-for="item in $store.state.taskModalData.comments" :key="item.id">
								<div class="mw-action__ava">
									<div class="txt" v-if="item.photoUrl.trim() === ''">
										{{ item.firstName.charAt(0) + item.lastName.charAt(0) }}
									</div>
									<img v-else :src="$store.state.url + item.photoUrl" alt="">
								</div>
								...
								<div class="mw-action__content">
									<div class="mw-action__content__top align-items-center">
										...
										<span class="mw-action__type"
											  :class="[
												  {'danger': item.commentType.value === $store.state.TASK_COMMENT_TYPE.PROBLEM},
												  {'success': item.commentType.value === $store.state.TASK_COMMENT_TYPE.SOLUTION}
											  ]">
											{{
												(item.commentType.value === $store.state.TASK_COMMENT_TYPE.INFORMATION ? 'Информация' : (item.commentType.value === $store.state.TASK_COMMENT_TYPE.PROBLEM ? 'Проблема' : 'Решение'))
											}}
										</span>

										<div class="d-flex" v-if="item.userId === $store.state.userProfile.id">
											<button type="button" class="btn btn-sm az_r_menu_ttl_btn" @click="CopyObjectForEdit(item)">
												<b class="fa fa-pencil"></b>
											</button>

											<button type="button" class="btn btn-sm az_r_menu_ttl_btn" @click="DeleteComment(item.id)">
									...
									<p class="mw-action__content__comment">
										{{ item.commentText }}
										<strong v-for="item2 in item.members" :title="item2.employee.firstName + ' ' + item2.employee.lastName">
											@{{ item2.employee.userName }}
										</strong>
									</p>


									<div class="action__content__top__form mt-3" v-if="item.id === commentEditModal.id">
										<form @submit.prevent="EditComment">
											...
											<div class="action-save">
												<div class="d-flex align-items-center">
													<div class="form-group mb-0">
														<select class="form-control" v-model="commentEditModal.typeId">
															<option v-for="item in $store.state.projectData.taskCommentTypes" :key="item.id" :value="item.id">
																<template v-if="item.value === $store.state.TASK_COMMENT_TYPE.PROBLEM">{{$t('problem')}}</template>
																<template v-else-if="item.value === $store.state.TASK_COMMENT_TYPE.SOLUTION">{{$t('solution')}}</template>
																<template v-else>{{$t('information')}}</template>
													...
													<div class="member-invite custom-dropdown__main us-n" data-custom-drop>
														<div class="mw-checklist__findUser button-effect mt-0 d-flex align-items-center" data-custom-drop-btn>
															@{{$t('workers')}}
															<strong class="num" v-if="commentEditModal.employee.length > 0">
															...
															<div class="member-invite__content">
																...
																<div class="d-flex flex-wrap">
																	...
																	<label class="boardofusers__item mb-0 cur-p"
																		   ...
																		   :title="item.employee.firstName + ' ' + item.employee.lastName"
																	>
																		<input type="checkbox" :value="item.employee.id" hidden v-model="commentEditModal.employee">
												...
												<div class="d-flex">
													<button type="button" class="modal-btn__save cancel button-effect mr-3" @click="ClearEditData">{{$t('cancel')}}
													</button>
													<button type="submit" class="modal-btn__save button-effect">{{$t('save')}}</button>
												</div>
											</div>
						...
						<ul class="mw-submenu mw-action__main" v-if="can('TASK_ACTION_READ')">
							<li class="mw-action__item" v-for="item in $store.state.taskModalData.actions" :key="item.id">
								<div class="mw-action__ava">
									<div class="txt" v-if="item.photoUrl.trim() === ''">
										{{ item.firstName.charAt(0) + item.lastName.charAt(0) }}
									</div>
